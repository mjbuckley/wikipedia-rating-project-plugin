...
// Accepts two parameters.  Be sure that $test_title has magic quote escaping removed or else Wikipedia api could get bad data.
function wrp_wiki_test( $test_title, $test_lastrevid ) {

  // No lastrevid given, so verify given title is lefit and then grab current lastrevid.
  ...
  if (empty($test_lastrevid)) {
    ...
    if( is_array($title_response) && !is_wp_error($title_response) ) {  // Verify response is in form we expect and not WP Error
      ...
      // $pre_info = isset( $title_decoded["query"]["pages"] ) ? $title_decoded["query"]["pages"] : false;
