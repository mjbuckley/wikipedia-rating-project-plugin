...
function wrp_wiki_test( $new_title_value, $new_lastrevid ) {

  // No lastrevid given so grab the current one

  if (empty($new_lastrevid)) {
    ...
    $encode_title = rawurlencode($new_title_value);
    ...
    if( is_array($title_response) ) {  // Verify response is in form we expect
      ...
      // Also grab lastrevid.
        ...
        if ($disambiguation && ($redirect || $normalization)) {
          ...
          $message = "Title has been changed from '{$new_title_value}' to '{$new_title}.'  Lastrevid is: {$lastrevid}.  Also, This looks like it might be a disambiguation page.";
        ...
        elseif ($redirect || $normalization) {
          ...
          $new_title = $disambiguation_test['title'];
          $message = "Title has been changed from '{$new_title_value}' to '{$new_title}.'  Lastrevid is: {$lastrevid}.";
          return array( 'error' => false, 'lastrevid' => $lastrevid, 'title' => $new_title, 'pageid' => $pageid, 'message' => $message );
        }
        elseif ($disambiguation) {
          ...
          return array( 'error' => false, 'lastrevid' => $lastrevid, 'title' => $new_title_value, 'pageid' => $pageid, 'message' => $message );
  ...
  // Lastrevid has been given, so check that it is valid and that it matches the given title.
    ...
    $encode_lastrevid = rawurlencode($new_lastrevid);
    ...
    // Lastrevid exists
      ...
      if (strtolower($new_title_value) !== strtolower($title)) {
      ...
      // All is basically good.  Check for differences in capitalization, the possiblitiy of being a disambiguation page, and grab pageid.
        ...
        if ($new_title_value !== $title) { 
          ...
          if (array_key_exists("categories", $info)) {
            $message = "This looks like it might be a disambiguation page.  Title has been changed to from '{$new_title_value}' to '{$title}'.";
            return array( 'error' => false, 'lastrevid' => $new_lastrevid, 'title' => $title, 'pageid' => $pageid, 'message' => $message );
          }

          // Titles differ in capitalization, but all else good.  Save, but alert user.
          else {
            $message = "Title has been changed from '{$new_title_value}' to '{$title}'.";
            return array( 'error' => false, 'lastrevid' => $new_lastrevid, 'title' => $title, 'pageid' => $pageid, 'message' => $message );
          }
        }
        ...
        // Titles are a perfect match.
          ...
          if (array_key_exists("categories", $info)) {
            ...
            return array( 'error' => false, 'lastrevid' => $new_lastrevid, 'title' => $new_title_value, 'pageid' => $pageid, 'message' => $message );
          }
